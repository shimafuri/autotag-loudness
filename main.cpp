...
void calculate_lufs(const char* s, std::string& lufs_integrated, std::string& lufs_max, std::string& lufs_min, std::string& lufs_range) {
    ...
    if (fs::is_regular_file(p)) {
      ...
      if (!f.isNull() && f.tag()) { // 音楽ファイルかつタグ情報があった場合
        ...
        if (p.extension() == ".m4a" || p.extension() == ".mp4") {
          ...
          // ラウドネス関連タグが存在しない場合のみ、ラウドネス関連タグを計算して書き込み
          if (!items.contains(TagLib::String("----:com.apple.iTunes:lufs_integrated"))) {
            calculate_lufs(
              s,
              lufs_integrated,
              lufs_max,
          ...
          // ラウドネス関連タグが存在しない場合のみ、ラウドネス関連タグを計算して書き込み
          if (!props.contains(TagLib::String("lufs_integrated"))) {
            calculate_lufs(
              s,
              lufs_integrated,
              lufs_max,
